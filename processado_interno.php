...
<body>
    ...
    <section>
        <h4 style="margin-top: 60px" class="text-center mb-4 text-primary" id="titulo-pagina">Registro Interno de Materiais processados</h4>
        ...
        <?php } ?>
            <div class="container" style="background: rgba(150,150,150,0.2);border-radius: 8px 8px 0px 0px">
              ...
              <div class="col-md-6">
                ...
                <a href="gera_pdf.php?proce_interno" class="btn btn-danger ml-2 mb-3 text-light" id="add-button-saida">
                  RELATÓRIO
                </a>
              </div>
              <div class="col-md-6">
                <form action="processado_interno.php" method="GET">
                  <div class="form-row">
                    <div class="input-group col-md-6">
                      <select name="tipobusca" class="form-control mb-2" id="FormControlSelectTypeSearch" onchange="trocatipoProcessadoInterno()">
                        <option value="0" hidden>Selecione método busca</option>
                        <option value="b.descricao">Material</option>
                        <option value="c.lote">Lote</option>
                        <option value="c.numero_do_ciclo">Número do ciclo</option>
                        <option value="c.pressao">Pressão</option>
                        <option value="c.temperatura_interna">
                        Temperatura</option>
                        <option value="c.data">
                        Data</option>
                      </select>
        ...
        <?php foreach ($lista as $key => $dados){ ?>
                <tr class="">
                  <td scope="col"><input class="form-control" type="checkbox" value="<?= $dados['id_material'] ?>, <?= $dados['quantidade'] ?>, <?= $dados['id_processando_material'] ?>" class="form-check-input" name="check-material-id"></td>
                  
        <?php if($_SESSION['perfil'] == 'admin'){ ?>
                  <td>
                    <button onclick="deletaProceinterno(<?= $dados['id_processando_material'] ?>,<?= $_SESSION['id_hospital'] ?>,<?= $dados['id_kit_recebido'] ?>)" class="btn btn-danger">
                      <i class="far fa-times-circle"></i>
                    </button>
                  </td>
        <?php } ?>

                  <td scope="col"><?= $dados['id_material'] ?></td>
                  <td scope="col"><?= $dados['descricao'] ?></td>
      ...
      $('#add-button-saida').click(function(e){
        ...
        for (var i=0;i<materialIdQtd.length;i++){ 
          if (materialIdQtd[i].checked == true){ 
            ...
            materialArray.push({"id": meteriais[0], "qtd": meteriais[1], "id_processado_material": meteriais[2]});
        ...
        $.ajax({
              ...
              data: {
                  id: materialArray,
                  qtd: '',
                  local: 'material_processando_interno',
                  posicao: 'null'
              },
              dataType: 'json',
              success: function(response){
                if(response.success === true){
                  window.location.href = "cadastro_saido_interno.php"
                  return;
                }else{
                  var showError = document.getElementById("show-error");
