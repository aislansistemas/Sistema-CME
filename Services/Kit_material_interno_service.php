<?php
	...
	class Kit_mat_inter_service{
		...
		public function salvaKitInterno(){
			...
			(id_recebido,id_hospital,id_material)
			values(:id_recebido, :id_hospital, :id_material)";
		...
		public function alterarQtdKitInterno(){
			$query="update tb_kit_material_recebido_interno SET `status` = :status WHERE (`id` = :id and `id_hospital` = :id_hospital);";
			$stmt=$this->conexao->prepare($query);
